...
| Database Schema | **Done** | 5 張表已建立 + RLS + triggers + indexes + storage bucket (`001_initial_schema.sql`) |
...
| TypeScript Types | **Done** | `src/lib/supabase/types.ts` — 完整 Database 型別定義 |
...
3. **Mock Data** — Daily Feed、Connections、Chat、Profile 均使用寫死的 mock data
4. **Responsive** — 部分頁面 (特別是 `/daily`) 在手機尺寸可能需要調整
...
| Phase E: Profile + Finishing | Tasks 14-15 (profile API, 24hr auto-disconnect) | Pending |
