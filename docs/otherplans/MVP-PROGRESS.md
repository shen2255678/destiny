...
| Migration 005 | **Planned** | Rectification 欄位（accuracy_type, window_start/end, rectification_status, current_confidence, active_range, calibrated_time, is_boundary_case, dealbreakers）+ `rectification_events` 新表 + 修正 `birth_time` evening constraint |
...
| Rectification Data Layer | **Specced** | Migration 005 + birth-data API 更新 + `GET/POST /api/rectification/` endpoints。設計見 `docs/plans/2026-02-18-rectification-data-layer-design.md`，spec 見 `docs/Dynamic_BirthTimeRectification_Spec.md` |
...
| Python astro-service | **Railway** | Hobby ($5/月) | Not deployed | 支援 pyswisseph C native lib；需加 `Procfile` |
| Database + Auth + Storage | **Supabase** | Free tier | **Live** | `masninqgihbazjirweiy.supabase.co` |
| AI API (Claude / Gemini / OpenAI) | Anthropic / Google / OpenAI | Pay-as-you-go | Not integrated | 從 Vercel API routes 呼叫；注意 10s timeout（見備註）|

### ⚠️ AI API Timeout 備註

Vercel Hobby 方案 serverless function timeout 為 **10 秒**。

| AI 功能 | 預估時間 | 對策 |
|---------|---------|------|
| 標籤生成、破冰題、校正問題 | < 5s | 直接呼叫，無問題 |
| 靈魂原型報告、深度分析 | 10–30s | ① 改用 **Streaming**（`ReadableStream`，見 `docs/DEPLOYMENT.md §4.2`）**或** ② 將長生成移到 **Railway**（Python，無 timeout 限制） |
| 若長期需要長生成 | — | 升級 Vercel Pro（$20/月，60s timeout） |
...
6. **Rectification Data Layer（Phase B.5）** — Migration 005 + birth-data API 更新 + Rectification quiz endpoints ← **NEXT**
   - 設計文件：`docs/plans/2026-02-18-rectification-data-layer-design.md`
   - 主要任務：Migration SQL → types.ts 更新 → birth-data API 擴充 → Onboarding UX 改版 → Rectification quiz API
7. **實作 Daily Matching** — Python cron + match algorithm
8. **串接 AI/LLM** — 動態原型生成、變色龍標籤
9. **實作 Chat** — Supabase Realtime WebSocket
10. **Progressive Unlock Logic** — DB trigger + frontend unlock
