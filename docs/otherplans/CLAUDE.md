...
**Status:** Active development — Phase A (Onboarding) ✅, Phase B (Astro Microservice) ✅, Phase B.5 (Rectification Data Layer) specced → **NEXT**, Phase C (Daily Matching) pending.
...
- `docs/DEPLOYMENT.md` — Hosting guide: Vercel (Next.js) + Railway (Python) + AI API strategy
- `docs/Dynamic_BirthTimeRectification_Spec.md` — Full spec for Dynamic Birth Time Rectification system
- `docs/plans/` — Implementation design docs (one per feature, format: `YYYY-MM-DD-<topic>-design.md`)
...
```
1. Read docs/MVP-PROGRESS.md → find the "NEXT" item in Next Steps
2. Read the corresponding design doc in docs/plans/ (linked in MVP-PROGRESS)
3. Check §"Files to Modify" in the design doc for the ordered task list
```

### Step 2 — Execute in order (TDD)
For each file in the "Files to Modify" list:
```
1. Write failing test first (Vitest for TS, pytest for Python)
...
```
1. Run full test suite: cd destiny-app && npx vitest run
2. Update docs/MVP-PROGRESS.md: mark tasks Done, add test counts
3. Update "Next Steps" to point to the new next task
```

### Current Next Task: Rectification Data Layer (Phase B.5)

Design doc: `docs/plans/2026-02-18-rectification-data-layer-design.md`

Execution order (from design doc §8 "Files to Modify"):

| # | File | Type | What to do |
|---|------|------|------------|
| 1 | `supabase/migrations/005_rectification.sql` | **New** | Run SQL in §2.1 of design doc; push to Supabase |
| 2 | `src/lib/supabase/types.ts` | Edit | Add new columns + `rectification_events` table types (design doc §7) |
| 3 | `src/__tests__/api/onboarding-birth-data.test.ts` | Edit | Add tests: PRECISE/TWO_HOUR_SLOT/FUZZY_DAY + boundary flag + event log |
| 4 | `src/app/api/onboarding/birth-data/route.ts` | Edit | Accept new fields, init rectification state, detect boundary, log event (design doc §4.3) |
| 5 | `src/app/onboarding/birth-data/page.tsx` | Rewrite | 4-card precision UX + 12-slot grid + FUZZY_DAY 3-option (design doc §3) |
| 6 | `src/app/api/rectification/next-question/route.ts` | **New** | Spec §14.1 — select next question by boundary priority |
| 7 | `src/app/api/rectification/answer/route.ts` | **New** | Spec §14.2 — applyNegativeFilter, update confidence, log event |
| 8 | `docs/MVP-PROGRESS.md` | Edit | Mark done, update test counts |
